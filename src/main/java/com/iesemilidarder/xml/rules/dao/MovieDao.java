...
@Component
public class MovieDao extends AbstractProjectDao {
    ...
    public List<Document> getMoviesByCountry(String... country) {
        List<Document> movies = new ArrayList<>();
        //Arrays.asList(match(eq("countries", "Kosovo")))
        List<String> codes = Arrays.asList(country);
        //TODO TASK 1> Ticket: Projection - implement the query and projection required by the unit test
        Document queryFilter = new Document("countries", new Document("$all", codes));
        moviesCollection
                .find(queryFilter)
                //.limit(1)
    ...
    public List<Document> getMoviesByCast(String sortKey, int limit, int skip, String... cast) {
        //TODO TASK 2> Ticket: Subfield Text Search - implement the expected cast
    ...
    public List<Document> getMoviesByGenre(String sortKey, int limit, int skip, String... genres) {
        ...
        /*Bson limitFilter = Aggregates.limit(limit);
        Bson skipFilter = Aggregates.skip(skip);*/
